...
const sudoku = function (board) {
  // board로 반복문을 돌린다
  for (let xIdx = 0; xIdx < board.length; xIdx++) {
    // yIdx에 board[i]에서 0이 처음으로 등장하는 인덱스를 할당한다.
    let yIdx = board[xIdx].indexOf(0);
    // dfs에 xIdx,yIdx를 넣어준다.
    // yIdx가 존쟈하면 재귀를 사용한다.
    if (yIdx >= 0) dfsSdoku(board, xIdx, yIdx);
  }
  return board;
};

const dfsSdoku = (board, xIdx, yIdx) => {
  // // board[xIdx]를 반복문으로 돌린다.
  // for (let yIdx = 0; yIdx < board[xIdx]; yIdx++) {
  //   // 종결조건 x,y 좌표가 board.length보다 크면 종결
  //   if (board[xIdx][yIdx] !== 0) continue;
  // Stack 배열 한 개를 선언한다.
  let stack = [];
  ...
  let candidate = getCandidateNums(board, xIdx, yIdx);
...
const getCandidateNums = (board, colIdx, rowIdx) => {
